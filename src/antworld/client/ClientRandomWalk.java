...
public class ClientRandomWalk
{
  ...
  private PacketToServer chooseActionsOfAllAnts(PacketToClient packetIn)
  {
    ...
    // For now, I just add all ants to waterChain.
    for( AntData ant : packetIn.myAntList ){ waterChain.addAnt(ant); }
    waterChain.setAntActions( packetIn ); // Then we set all their actions.

    // experimental clustering/dispersal code
    //for( AntData ant : packetIn.myAntList ){ hiveMind.addAnt(ant); }
    //hiveMind.setAntActions( packetIn ); // Then we set all their actions.
    //AttractorField.updateField( packetIn );
  ...
  public void setupNest(PacketToClient packetIn)
  {
    ...
    waterChain = new WaterChain( centerX, centerY );
    //hiveMind = new HiveMind( centerX, centerY );
